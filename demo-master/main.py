# this file was created by Steven Goldenberg
...
Game ideas:
- instead of platforms make them walls moving down with gaps to jump through
- add a second player to race, one using WASD and the other using arrowkeys - current 
- remember and display high scores 
- add solo and vs modes - current 
- change background 

Bugs:
- trouble getting collision and a new sprite for player2 
...
class Game:
    ...
    def update(self):
        ...
        mob_hits = pg.sprite.spritecollide(self.player, self.mobs, self.player2, False)
        if mob_hits:
            ...
            if self.player2.pos.y - 35 < mob_hits[0].rect_top:
                print("hit top")
                print("player2 is " + str(self.player2.pos.y))
                print("mob is " + str(mob_hits[0].rect_top))
                self.player2.vel.y = -BOOST_POWER
            else:
                print("player is " + str(self.player.pos.y))
                print("mob is " + str(mob_hits[0].rect_top))
                self.playing = False
        ...
        if self.player.vel.y > 0:
            hits = pg.sprite.spritecollide(self.player, self.platforms, self.player2, False)
            if hits:
        ...
        if self.player2.vel.y > 0:
            hits = pg.sprite.spritecollide(self.player, self.platforms, self.player, False)
            if hits:
                # set var to be current hit in list to find which to 'pop' to when two or more collide with player
                find_lowest = hits[0]
                for hit in hits:
                ...
                if self.player2.pos.x < find_lowest.rect.right + 10 and self.player2.pos.x > find_lowest.rect.left - 10:
                    if self.player2.pos.y < find_lowest.rect.centery:
                        self.player2.pos.y = find_lowest.rect.top
        ...
        if self.player2.rect.top <= HEIGHT / 4:    
            self.player2.pos.y += max(abs(self.player2.vel.y), 2)
            
            for mob in self.mobs:
                # creates slight scroll based on player y velocity
                mob.rect.y += max(abs(self.player.vel.y), 2)
                mob.rect.y += max(abs(self.player2.vel.y), 2)
            for plat in self.platforms:
                # creates slight scroll based on player y velocity
                plat.rect.y += max(abs(self.player.vel.y), 2)
                plat.rect.y += max(abs(self.player2.vel.y), 2)
                if plat.rect.top >= HEIGHT + 40:
                    plat.kill()
                    self.score += 10
        # if player hits a power up
        pow_hits = pg.sprite.spritecollide(self.player, self.powerups, self.player2, True)
        for pow in pow_hits:
            if pow.type == 'boost':
